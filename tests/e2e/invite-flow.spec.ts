...
// listUsers() без перпейджа шлёт per_page='' → новый GoTrue возвращает 0 юзеров.
// Используем RPC get_user_id_by_email (как в get-invite) — обходит это ограничение.
async function cleanup() {
  const { data: uid } = await svc.rpc('get_user_id_by_email', { p_email: TEST_EMAIL })
  if (uid) {
    await svc.from('tenant_members').delete().eq('user_id', uid)
...
test('accept-invite new-user: создаёт юзера, membership, проставляет accepted_at', async () => {
  ...
  // RPC get_user_id_by_email вместо listUsers(): listUsers без per_page шлёт
  // per_page='' → новый GoTrue возвращает пустой массив (интерпретирует как 0).
  const { data: createdId } = await svc.rpc('get_user_id_by_email', { p_email: TEST_EMAIL })
  expect(createdId, 'auth-юзер TEST_EMAIL должен быть создан').toBeTruthy()

  // tenant_members: роль «Сотрудник», demo-тенант.
  const { data: member } = await svc
    .from('tenant_members')
    .select('role_id, tenant_id')
    .eq('user_id', createdId as string)
    .eq('tenant_id', DEMO_TENANT_ID)
    .single()
