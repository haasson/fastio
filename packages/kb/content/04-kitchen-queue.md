...
> **Право «Готовка» (`kitchen.cook`).** Брать блюда в работу, отмечать готовыми и возвращать в очередь может только сотрудник с правом «Готовка». Без него (но с правом «Доступ к кухонной очереди») экран открывается в **режиме просмотра**: видна вся нагрузка и таймеры, но кнопки «Взять» / «Готово» / «Вернуть» скрыты. Так сборщик или менеджер следят за загрузкой кухни, не перехватывая блюда у поваров.
