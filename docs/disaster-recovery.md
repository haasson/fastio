...
## Сценарий 4: «Удалили фото блюд / storage corruption»

Storage (фото блюд, баннеры, gallery) хранится в `twS3:fastio-storage/`.
Weekly snapshot копируется в `twS3:fastio-backups/storage-YYYY-MM-DD/`
(retention 4 последних).
...
# Список snapshots
fastio-restore-storage.sh list

# Восстановить из последнего (самый частый случай)
fastio-restore-storage.sh latest

# Или конкретная дата
fastio-restore-storage.sh 2026-05-17

# Подтверждение: yes-restore-storage
```

**Семантика:** `rclone copy` (не `sync`) — файлы из snapshot перезаписывают
существующие с тем же путём. Файлы загруженные ПОСЛЕ snapshot (не было в backup)
**остаются как есть** — это fail-safe, чтобы случайный restore не стёр свежие
загрузки.

**Если нужен жёсткий "вернуть бакет в состояние snapshot ровно как было":**
```bash
# rclone sync — удаляет всё чего нет в snapshot
rclone sync twS3:fastio-backups/storage-YYYY-MM-DD/ twS3:fastio-storage/
```
⚠ Это удалит все файлы загруженные после snapshot. Используй только когда
уверен что нужен именно "rollback to a point in time" для storage.

---

## Сценарий 5: «Откат restore'а — выбрали не тот dump»
...
**Manual run:**
```bash
# PITR test до текущего момента
ssh root@109.71.242.205 'fastio-walg-restore-test.sh'

# PITR test до конкретной точки
ssh root@109.71.242.205 "fastio-walg-restore-test.sh --target-time '2026-05-20 14:25:00 UTC'"
```

**Доказательство что PITR реально работает (2026-05-20):**
- Test прогнан вручную после установки wal-g
- Time: 16 секунд
- Result: PASSED — 75 public-таблиц, 1 tenant, 1 auth.user (точное состояние prod)
- Лог: `/var/log/fastio-backup/walg-restore-test-20260520-070000.log`
