-- ═══════════════════════════════════════════════════════════════════════════════
-- 167: Fix billing_change_plan — module_configs.required_plan_key хранит уровень
-- (showcase/start/pro), а plans.key — полный ключ (retail-start, services-pro).
-- Старый JOIN p.key = mc.required_plan_key не находил совпадений, из-за чего
-- модульная проверка при даунгрейде молча пропускала всё. Сравниваем по tier
-- через sort_order: tier='start' → 1, tier='pro' → 2.
-- ═══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Tier sort_order: showcase=0, start=1, pro=2. Новый sort_order из выбранного плана.
  -- Модуль конфликтует если его required_plan_key (уровень) выше нового уровня.
  SELECT array_agg(mc.name)
  INTO v_conflicting_modules
  FROM module_configs mc
  WHERE mc.is_active = true
    AND (v_tenant.modules->>mc.key)::boolean IS TRUE
    AND CASE mc.required_plan_key
          WHEN 'showcase' THEN 0
          WHEN 'start' THEN 1
          WHEN 'pro' THEN 2
          ELSE 0
        END > v_new_plan.sort_order;
