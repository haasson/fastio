-- Migration 215: service snapshot in appointments
--
-- Проблема: appointments.service_id — FK на services. Если услугу удалили
-- или изменили цену — старые записи теряют имя/цену услуги.
--
-- Решение:
-- 1. Добавляем service_name + service_price — снапшот на момент создания записи.
-- 2. FK service_id меняем с ON DELETE RESTRICT на ON DELETE SET NULL:
--    удалённая услуга не ломает запись, service_id просто становится null.
-- 3. Обновляем RPC create_appointment / create_appointments_bulk:
--    принимают service_name + service_price и вставляют их в снапшот.

-- ─── Снапшот-колонки ──────────────────────────────────────

ALTER TABLE appointments
  ADD COLUMN service_name  text    NOT NULL DEFAULT '',
  ADD COLUMN service_price numeric NOT NULL DEFAULT 0;

-- ─── FK service_id: RESTRICT → SET NULL ──────────────────
...
DROP FUNCTION IF EXISTS public.create_appointment(
  ...
  timestamptz, timestamptz, appointment_status, text, boolean, boolean
);
...
-- p_items теперь: [{service_id, resource_id, starts_at, ends_at, service_name, service_price}]
