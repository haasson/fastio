-- Migration 206: appointments.customer_id — durable link to customers row.
--
-- Why: prior to this, "show my appointments" filtered by user_id OR
-- customer_phone snapshot. Two failure modes:
--   1) Customer changes phone in profile → loses access to past appointments.
--   2) Stranger types the same phone in a new booking → sees foreign records.
--
-- Fix: persist customer_id at create time, filter by it. Phone snapshots stay
-- in customer_name/customer_phone for audit and admin display, but they no
-- longer drive ownership.
--
-- Backfill: link existing rows by (tenant_id, user_id) ↔
-- customers(tenant_id, auth_user_id). Guest rows (user_id IS NULL) stay NULL.
...
UPDATE appointments a
 ...
 WHERE c.tenant_id = a.tenant_id
   AND c.auth_user_id = a.user_id
   AND a.user_id IS NOT NULL
   AND a.customer_id IS NULL;

-- ─── Re-create RPC with p_customer_id parameter ────────────────────────
...
REVOKE ALL ON FUNCTION public.create_appointment(
  uuid, uuid, uuid, uuid, uuid, uuid, text, text, timestamptz, timestamptz,
  appointment_status, text, boolean, boolean
) FROM PUBLIC;
