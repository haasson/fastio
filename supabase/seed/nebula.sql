-- =============================================================================
-- FASTIO SEED — NEBULA hookah lounge
-- =============================================================================
-- Before running: the owner account must already exist. Set the email below.
...
DECLARE
  ...
  -- bottle-only wines: no UUID needed (gen_random_uuid inline)
  ...
  -- Настойки Онегин (tincture modifier)
  ...
  -- Биттеры/Ликёры/Вермуты (spirit modifier)
  ...
  -- Order statuses
  os_accepted       uuid := gen_random_uuid();
  os_preparing      uuid := gen_random_uuid();
  os_served         uuid := gen_random_uuid();
  os_cancelled      uuid := gen_random_uuid();
...
BEGIN
  ...
  -- ============================================================
  DELETE FROM tenants WHERE slug = 'nebula';
  ...
  IF _owner_id IS NULL THEN
    RAISE EXCEPTION 'Owner not found. Set the correct email in the script.';
  END IF;


  -- ============================================================
  -- TENANT: NEBULA — hookah lounge
  -- ============================================================
  ...
  INSERT INTO tenants (
    ...
    -- theme
    '{"preset":"dark","primaryColor":"#8b5cf6","fontFamily":"system","headingFontFamily":"system","palette":null,"buttonRadius":"rounded","cardRadius":14,"cardShadow":"subtle","customThemes":[],"activeCustomId":null}'::jsonb,

    -- contacts
    '{"phoneMode":"shared","phone":"","email":"","address":"","instagram":"nebula_lounge","vk":null,"telegram":null,"whatsapp":null}'::jsonb,

    -- working_hours_schedule
    '{"default":{"open":"14:00","close":"02:00"},"days":{}}'::jsonb,

    -- site_layout
    '{"header":{"showNav":true,"navItems":[{"key":"menu","action":"scroll"},{"key":"about","action":"navigate"}],"showPhone":true,"showWorkingHours":true},"sections":{"categoryBar":{"enabled":true,"overflow":"scroll"},"hero":{"enabled":true,"size":"fullscreen","bgType":"gradient","overlayColor":"#000000","overlayOpacity":0.35,"contentPosition":5,"contentAlign":"center","gradientId":"diag-bp"},"banners":{"enabled":false,"displayMode":"single","autoplay":false,"autoplayInterval":4},"menu":{"enabled":true,"defaultView":"categories","tagDisplayMode":"both"},"gallery":{"enabled":false,"galleryIds":[]},"reviews":{"enabled":false},"delivery":{"enabled":false}},"sectionsOrder":["hero","categoryBar","menu"],"pages":["about"],"pageSettings":{"menu":{"defaultView":"categories","tagDisplayMode":"both"},"delivery":{"showMap":false,"descriptionMode":"auto"},"gallery":{"galleryIds":[]}}}'::jsonb,

    -- site_content
    '{"logo":null,"hero":{"bgUrl":null,"text":"NEBULA — кальянная с характером"},"about":{"coverUrl":null,"text":"NEBULA — hookah lounge."},"delivery":{"manualText":""}}'::jsonb,
  ...
  INSERT INTO order_statuses (id, tenant_id, name, group_type, position) VALUES
    (os_accepted,  t, 'Принят',    'new',         1),
    (os_preparing, t, 'Готовится', 'in_progress', 2),
    (os_served,    t, 'Подан',     'completed',   3),
    (os_cancelled, t, 'Отменён',   'cancelled',   4);
