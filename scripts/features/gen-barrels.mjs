#!/usr/bin/env node
// Автогенерация index.ts (barrel) для модулей в apps/admin/features/<X>/.
//
// Правила:
//   • Сканит подпапки: api/, composables/, utils/, stores/, columns/, config/
...
//       - имя совпадает с папкой → дефолтный экспорт модуля, не используется
...
//
// Usage:
//   node scripts/features/gen-barrels.mjs              # обновить (только маркированные)
//   node scripts/features/gen-barrels.mjs --check      # CI mode
//   node scripts/features/gen-barrels.mjs --force      # перезаписать ВСЕ
//   node scripts/features/gen-barrels.mjs --feature=reservations  # одна фича
...
const SCAN_SUBDIRS = ['api', 'composables', 'utils', 'stores', 'columns', 'config']
...
function buildBarrel(featureName, lines) {
  const header = [
    MARKER,
    `// Авто-сгенерировано scripts/features/gen-barrels.mjs.`,
