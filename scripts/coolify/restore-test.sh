...
# Тишина ≠ всё хорошо. Слать сообщение и при успехе — см. backup.sh.
tg_send() {
...
notify_failure() {
  ...
  tg_send "$(printf '🚨 <b>Проверка восстановления упала</b>\nОшибка: <code>%s</code>\nКод выхода: %d\nДлительность: %s\nЛог: <code>%s</code>\n\n<pre>%s</pre>' \
    "${err_line:-неизвестно}" "$rc" "$(fmt_duration)" "$LOG_FILE" "$tail_log")"
}
...
notify_success "$(printf '✅ <b>Месячная проверка восстановления</b>\nДамп: <code>%s</code>\nРазмер: %s\nДлительность: %s\nТаблиц в public: %s\nТенантов: %s, юзеров: %s\nВремя: %s Барнаул\nСледующая: 1-го числа след. месяца в 12:00' \
  "$LATEST" "$DUMP_SIZE" "$(fmt_duration)" "$PUBLIC_TABLES" "$TENANTS" "$USERS" "$(local_now)")"
trap - EXIT
cleanup
