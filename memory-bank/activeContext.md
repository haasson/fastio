...
## Recent Changes
- **Telegram-напоминания клиенту о записях** (2026-05-08, миграция 259+260):
  - Новая таблица `appointment_reminders(appointment_id, telegram_chat_id, remind_before_minutes, sent_at)` + RLS service-role only. Опции: за 30 мин / 2ч / 1д / 2д. Cron pg_cron каждую минуту дёргает admin-эндпоинт `/api/telegram/send-appointment-reminders` (URL и секрет в vault); claim-and-send через `UPDATE … WHERE sent_at IS NULL` + откат claim'а на 5xx/network для retry на следующей минуте.
  - **Token-based deep-link (single-use)**: вместо публикации UUID appointment'а в `t.me/<bot>?start=remind_<UUID>` — отдельная таблица `appointment_reminder_tokens` (PK token, FK appointment, expires_at = min(starts_at, now+7d)). Storefront-эндпоинт `/api/appointments/remind-token` выдаёт 22-символьный base64url-токен только владельцу записи. Webhook резолвит токен в appointment, после первого выбора минут — `DELETE FROM tokens` (single-use). Цель: enum по UUID невозможен, инфо-leak (service_name, дата) исключён.
  - Admin webhook (`auth-webhook.post.ts`) теперь принимает не только /start nonce (auth flow), но и /start remind_<token> + callback_query от inline-клавиатуры. Подписка на `callback_query` добавлена в `setWebhook` через `telegram-bot-setup.ts`.
  - Storefront: компонент `ApptReminderOffer.vue` показывается на success-экране чекаута. Две ветки: hasTelegram → POST `/api/appointments/remind-offer` шлёт сообщение в чат клиента, иначе deep-link на бот с предзагруженным токеном (фон-fetch на mount).
  - Shared: `packages/shared/src/utils/telegramReminderOptions.ts` — `REMINDER_OPTIONS`, `REMINDER_TOKEN_RE`, `REMINDER_UUID_RE`, `REMINDER_TOKEN_TTL_MS`, `buildReminderKeyboard(token)`, `buildReminderOfferText`, `formatAppointmentDateTime`, `escapeHtml`, `isValidReminderMinutes`. Дедуп форматтеров (5 мест) и keyboard-builder в одном месте.
  - Cleanup-cron'ы: «cleanup-old-appointment-reminders» (сутки старше 7 дней) и «cleanup-expired-reminder-tokens» (после `expires_at`).
