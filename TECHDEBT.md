...
## audit_logs: каскадное удаление дочек (combo_items, modifier_options) не логируется

Триггер `fn_audit` на дочерних таблицах резолвит `tenant_id` через родителя (`SELECT tenant_id FROM <parent> WHERE id = <fk>`). При hard-delete родителя с `ON DELETE CASCADE` AFTER-DELETE-триггер дочки выполняется, когда родитель уже удалён → lookup вернёт NULL → orphan-guard → удаление дочки **молча не логируется**. Затрагивает: `combo_items` (при `DELETE FROM combos`) и `modifier_options` (при hard-delete `modifier_groups`). **Важно:** удаление самого родителя (combo/группа) при этом логируется штатно — то есть «что произошло» на уровне сущности в журнале есть, теряется только гранулярная строка по каждой дочке. Прямое удаление дочки (не каскадом) логируется нормально. Малоценно (у дочек нет/слабый name), но дыра в покрытии. Фикс при необходимости: snapshot tenant_id из `OLD` до каскада, либо логировать удаление родителя с составом в payload. Миграция 321.
