 ...
 * Принимает массив правил (по email, по IP, и т.д.) — все проверяются параллельно
 * и при провале ЛЮБОГО кидается 429 с заданным сообщением.
 *
 * Используется в auth-endpoints (login/register/forgot-password/reset-password),
 * где нужен durable лимит (in-memory от createRateLimiter не годится — не переживёт
 * рестарт и не шарится между инстансами).
 */
type RateLimitRule = {
  key: string
  max: number
