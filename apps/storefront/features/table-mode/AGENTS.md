...
| Файл | Что внутри |
|---|---|
| `stores/table.ts` | `useTableStore` — state стола (id, name, `checkItems`, `draftItems`, computed `checkTotal/itemCount/draftCount/draftTotal`) + `setTable/setCheckItems/addDraftItem/updateDraftQty/removeDraftItem/removeDraftByKeys/clearDraft/restoreDraft/clear`. Драфт дедупится по `draftSignature` (qty++ для одинаковой позиции; строже корзины — учитывает comboId). `removeDraftByKeys(Set)` удаляет ровно отправленные позиции после успешного POST (snapshot против потери добавленного во время отправки). **Драфт персистится в `sessionStorage`** (один ключ `table-draft`, `tableId` внутри payload — `restoreDraft()` сверяет id, чтобы при смене QR-стола в той же вкладке не воскресить чужой драфт; переживает рефреш, чистится со вкладкой) — `restoreDraft()` зови из page в `onMounted` (client-only, иначе hydration mismatch). **Персист — в самих экшенах мутаций, НЕ через `watch(draftItems)`**: Pinia для setup-стора применяет SSR-состояние ПОСЛЕ регистрации watch'ей, поэтому deep-watch срабатывал на гидрации и затирал сохранённый драфт пустым серверным `[]`. Гард — `typeof sessionStorage`, не `import.meta.client` (тестируемо + фиче-детект). Экспортирует `aggregateCheckItems(items)` — дедуп позиций чека для отображения (имя+модификаторы+аддоны+убранные, qty суммируется), как `tablesApi.loadSums` в админке. |
...
- ❌ Кешировать `checkItems` в localStorage — это live-state стола, кеш нерелевантен (другой гость на следующий день получит чужой чек). Драфт — другое дело: он персистится в `sessionStorage` (живёт только в пределах вкладки), это ОК.
- ⚠️ Realtime UPDATE для гостя (anon) работает ТОЛЬКО если у таблицы `REPLICA IDENTITY FULL`. RLS-политики `anon_select_dine_in_*` ссылаются на `order_id` (не PK) — при DEFAULT realtime не может проверить политику против старого кортежа и роняет UPDATE-события (статус кухни не доезжал, миграция 310). Добавляешь realtime-таблицу с RLS на не-PK колонках → ставь `REPLICA IDENTITY FULL`.
