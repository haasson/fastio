...
| `components/CallWaiterButton.vue` | Кнопка вызова официанта в header layout. Сама грузит `/api/table/[id]/call-types`, при >1 типов показывает FsDrawer-выбор. 30s-cooldown на клиенте + durable rate-limit на сервере. |
...
| `server/api/table/[id]/call-types.get.ts` | Список типов вызова официанта (для UI выбора) |
| `server/api/table/[id]/call.post.ts` | Insert в `table_calls`. Cookie guard + rate-limit 1/30s + опц. `callTypeId` |
