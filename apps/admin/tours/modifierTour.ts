...
export const getModifierTourSteps = (): TourStep[] => [
  ...
  }),
  {
    ...
    popover: {
      title: 'Влияет на вес',
      description: 'Включается, если разные варианты группы меняют итоговый вес блюда. Например, пицца 25 см и 35 см весят по-разному.<br><br>Нажмите «Далее» — включим этот режим и посмотрим как задать вес.',
      side: 'right',
      align: 'start',
    },
    onNext: async () => {
  ...
  formField({
    ...
    title: 'Опции',
    description: 'Это варианты внутри группы. Для «Размера» — «Маленькая», «Средняя», «Большая».<br><br>Рядом с каждой опцией — поле веса в граммах или миллилитрах. Доплату к цене можно задать позже, прямо в форме редактирования блюда.<br><br>Порядок опций меняется перетаскиванием за иконку.',
  }),
  saveButton({
