...
const EVENT_TEXT_FORMATTERS: Partial<Record<OrderEventType, (meta: EventMeta) => string>> = {
  ...
  items_added: (m) => {
    const items = Array.isArray(m.items) ? m.items as Array<{ name?: string; quantity?: number }> : []
    const text = items.map((i) => `${i.name ?? '?'} × ${i.quantity ?? 1}`).join(', ')

    return `Добавлены позиции: ${text || '—'}`
  },
