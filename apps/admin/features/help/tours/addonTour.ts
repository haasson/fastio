...
export const getAddonTourSteps = (): TourStep[] => [
  ...
  formField({
    ...
    description: 'Добавки, которые войдут в этот набор. Можно отметить любое количество из созданных ранее. Когда пресет назначен на блюдо, все эти добавки автоматически становятся доступны гостю.',
  }),
  saveButton({
    target: 'preset-save',
    title: 'Сохранить пресет',
    description: 'После сохранения пресет появится в форме редактирования блюда в разделе «Добавки» — достаточно выбрать его, и все нужные добавки подключатся разом.',
  }),
]
